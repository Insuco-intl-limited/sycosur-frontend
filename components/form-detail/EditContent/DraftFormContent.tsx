...
export function DraftFormContent({ formId, projectId }: DraftFormContentProps) {
  ...
  const handlePublishDraft = async (version?: string) => {
    try {
      ...
      toast.success(t("msg.success.draftPublishedSuccess"));
    } catch (error) {
      console.log(error)
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-4"> {/*TODO: set to 2 columns on md screens after presentation*/}
        ...
        <div className="rounded-md border p-4">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold mb-3">{t("sections.readyToPublish")}</h3>

              {/* Clear, actionable description */}
              <Alert className="mb-4">
                <AlertTitle>{t("sections.whatWillHappen")}</AlertTitle>
                <AlertDescription>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    <li>{t("sections.publishDraftInfo")}</li>
                    <li>{t("sections.deleteDraftInfo")}</li>
                  </ul>
                </AlertDescription>
              </Alert>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  disabled={true}  /* TODO: Set this to isDeleting to disable the button while deleting */
                >
                  <TrashIcon className="w-4 h-4" />
                  {isDeleting ? t("sections.deleting") : t("sections.deleteDraft")}
                </button>
                ...
                >
                  ...
                  {isPublishing ? t("sections.publishing") : t("sections.publishDraftButton")}
                </button>
              </div>
            </div>
          </div>
